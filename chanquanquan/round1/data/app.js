...
function legacyMetricCard(label, item, rule) {
  if (!item) {
    ...
    return `<article class="legacy-card legacy-baseline"><div class="legacy-card-head"><strong>${esc(label)}</strong>${info(rule + ' 此值为全轮次基线，不随问题类型或列表筛选变化；当前类型没有独立冻结分母。')}</div><div class="legacy-primary"><span>全轮次基线</span><b>${esc(baseline?.[0] || '—')}</b><small>${esc((baseline?.[1] || '未知').replace(' · 全轮次冻结基线', ''))}</small></div></article>`;
  }
  ...
  return `<article class="legacy-card legacy-live"><div class="legacy-card-head"><strong>${esc(label)}</strong>${info(item.rule || rule)}</div><div class="legacy-primary"><span>本类型真实值</span><b>${esc(value)}</b><small>${esc(meta)}</small></div></article>`;
}
...
function renderMetrics() {
  ...
  document.getElementById('metric-note').textContent = '指标按所选问题类型展示；标注“全轮次基线”的值不随类型切换。列表筛选不改变指标，未知单列。';
